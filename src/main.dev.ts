...
function createWorkerWindow() {
  ...
  // }
  workerWindow.loadURL(`file://${__dirname}/worker/work.html`);
}
...
const createWindow = async () => {
  if (
    ...
    // todo: add the extension: https://github.com/electron-react-boilerplate/electron-react-boilerplate/issues/2788
...
function createTray() {
  ...
  // 系统托盘图标
  iconPath = `${getAssetPath()}/icon_16x16@2x.png`;
  ...
  if (process.platform === 'win32') {
    iconPath = `${getAssetPath()}\\logo.ico`;
  ...
  function resetBounds() {
    ...
    if (process.platform === 'win32') {
      view.setBounds({
        x: 300,
        y: 62,
        width: mainWindowBounds.width - 300,
        height: mainWindowBounds.height - 102,
      });
    } else if (process.platform === 'darwin') {
      view.setBounds({
        x: 300,
        y: 62,
        width: mainWindowBounds.width - 300,
        height: mainWindowBounds.height - 62,
      });
    } else {
